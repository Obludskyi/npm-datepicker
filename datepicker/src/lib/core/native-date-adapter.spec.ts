...
import { DEC, FEB, JAN, MAR } from '@obludskyi/testing';
...
describe('NativeDateAdapter', () => {
  ...
  beforeEach(
    inject([DateAdapter, Platform], (dateAdapter: NativeDateAdapter, _platform: Platform) => {
      adapter = dateAdapter;
      platform = _platform;
  ...
  it('should throw when attempting to format invalid date', () => {
    expect(() => adapter.format(new Date(NaN), {})).toThrowError(/NativeDateAdapter: Cannot format invalid date\./);
  });
  ...
  it('should clamp date at lower bound', () => {
    expect(adapter.clampDate(new Date(2017, JAN, 1), new Date(2018, JAN, 1), new Date(2019, JAN, 1))).toEqual(
      new Date(2018, JAN, 1)
    );
  });

  it('should clamp date at upper bound', () => {
    expect(adapter.clampDate(new Date(2020, JAN, 1), new Date(2018, JAN, 1), new Date(2019, JAN, 1))).toEqual(
      new Date(2019, JAN, 1)
    );
  });

  it('should clamp date already within bounds', () => {
    expect(adapter.clampDate(new Date(2018, FEB, 1), new Date(2018, JAN, 1), new Date(2019, JAN, 1))).toEqual(
      new Date(2018, FEB, 1)
    );
  });
...
describe('NativeDateAdapter with MAT_DATE_LOCALE override', () => {
  ...
  beforeEach(
    inject([DateAdapter], (d: NativeDateAdapter) => {
      adapter = d;
    })
  );
